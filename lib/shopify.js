...
export async function fetchCustomers({ cursor = null, queryStr = "" } = {}) {
  ...
  while (attempt < maxRetries) {
    try {
      ...
      if (errors?.some((e) => e.extensions?.code === "THROTTLED")) {
         console.warn(`Shopify Throttled (Attempt ${attempt + 1}/${maxRetries}). Retrying...`);
      ...
      const customers = data.customers.edges.map(({ node }) => {
        ...
        for (const edge of node.orders?.edges || []) {
          if (!fallbackPhone) {
            fallbackPhone = edge.node.shippingAddress?.phone || edge.node.billingAddress?.phone || "";
          }
  ...
  clearTimeout(timeoutId);
  throw lastError || new Error("Failed to fetch customers after multiple attempts.");
}

...
export async function GET(request) {
  ...
  try {
    ...
    console.warn(
      "MongoDB not reachable, falling back to direct Shopify fetch:",
      dbErr.message,
    );
  }
  ...
  try {
    ...
    // Get current user role and email from headers (set by middleware)
    ...
    if (useMongo) {
      ...
      if (count > 0) {
        ...
        if (agent) {
          query.note = { $regex: agent, $options: "i" };
        }
        if (status !== "all") query.followupStatus = status;
        if (dateFrom || dateTo) {
          query["lastOrder.processedAt"] = {};
          if (dateFrom)
            query["lastOrder.processedAt"].$gte = new Date(dateFrom);
    ...
    if (search) {
      ...
      if (sanitized)
        queryParts.push(
          `(first_name:*${sanitized}* OR last_name:*${sanitized}* OR email:*${sanitized}* OR phone:*${sanitized}*)`,
        );
    }
    ...
    if (agent) {
      filtered = filtered.filter(
        (c) => c.note && c.note.toLowerCase().includes(agent.toLowerCase()),
      );
    }
